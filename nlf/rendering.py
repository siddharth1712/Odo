...
class MeshViewer:
    def __init__(self, imshape, camera, color=None):
        self.imshape = imshape  # Save imshape, but don't create viewer yet
    ...
    def render(self, render_wireframe=False, RGBA=False):
        ...
        # ✨ Create a fresh OffscreenRenderer here
        r = pyrender.OffscreenRenderer(self.imshape[1], self.imshape[0])
        color_img, depth_img = r.render(self.scene, flags=flags)
        r.delete()  # ✨ Clean up immediately
...
def render(vertices, faces, camera, wireframe=False, colors=None, imshape=(800, 800)):
    ...
    for vertices_single in vertices:
        mesh = trimesh.Trimesh(vertices_single * np.array([1, -1, -1], dtype=np.float32), faces,vertex_colors=colors)
...
class Renderer:
    def __init__(self, imshape=None, faces=None, wireframe=False, return_depth=False, color=None,
        ...
        self.viewer = MeshViewer(
            imshape=imshape, camera=cameralib.Camera.from_fov(55, imshape), color=color)
        self.faces = faces
    ...
    def set_image_shape(self, imshape):
        self.viewer.imshape = imshape

    def render_meshes(
            self, trimeshes, camera, RGBA=False, wireframe=False, imshape=None):
        self.viewer.remove_meshes()
