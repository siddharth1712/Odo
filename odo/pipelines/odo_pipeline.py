...
class OdoPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        depth_image = self.prepare_controlnet_image(
                image=depth_image,
                width=width,
                height=height,
                batch_size=batch_size * num_images_per_prompt,
        ...
        reference_image = self._encode_vae_image(reference_image, generator=generator)
        ...
        with nullcontext() as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # ReferenceNet feature extraction. Skipped when reference_image is None
                # (the "w/o ReferenceNet" ablation) — reference_features stays None and the
                # ReshapeNet attention falls back to plain self-attention.
                if reference_image is not None:
                    down, reference_features = self.unet_encoder(reference_image, t, reference_prompt_embeds, return_dict=False)
                ...
                if self.do_classifier_free_guidance and reference_features is not None:
