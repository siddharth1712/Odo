...
class IPAttnProcessor_referencepass_noip(torch.nn.Module):
    r"""
    ...
    def __call__(
        ...
        if is_reference_pass or up_cnt is None:
            residual = hidden_states
            ...
            reference_feature = reference_image[up_cnt*3 + inside-1]
            reference_feature = rearrange(reference_feature, "b c h w -> b (h w) c").contiguous()
            # print(reference_feature.shape)
            # print(self.hidden_size)            
            c_key = self.to_k_c(reference_feature)
            c_value = self.to_v_c(reference_feature)
            ...
            #$$ attn_mask?  
            hidden_states_reference = F.scaled_dot_product_attention(
                query, c_key, c_value, attn_mask=None, dropout_p=0.0, is_causal=False
            )

            hidden_states_reference = hidden_states_reference.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
            hidden_states_reference = hidden_states_reference.to(query.dtype)

            hidden_states = hidden_states + self.scale * hidden_states_reference
...
class IPAttnProcessor_referencepass(torch.nn.Module):
    r"""
    Attention processor for IP-Adapater for PyTorch 2.0.
    Args:
        hidden_size (`int`):
...
class IPAttnProcessor_referencepass_extend(torch.nn.Module):
    r"""
    Attention processor for IP-Adapater for PyTorch 2.0.
    Args:
        hidden_size (`int`):
    ...
    def __call__(
        ...
        #     return hidden_states
            ...
            # # print(up_cnt*3 + inside)
            # print(inside_down)
            reference_feature = reference_image[inside_down]
            # print(reference_feature.shape)
            # if down_cnt is not None:
            #     # print("up_index")
            #     reference_feature = reference_image[down_cnt*3 + inside_down+1]
            #     # print(up_cnt*3 + inside_up)
            # elif mid_cnt is not None:
            #     reference_feature = reference_image[9]
            # else:            
            #     reference_feature = reference_image[11+up_cnt*3 + inside_up]
                # print("down_index")
...
class IPAttnProcessorMulti2_0_2(torch.nn.Module):
    ...
    def __call__(
        ...
        if is_reference_pass or up_cnt is None:
            ...
            # print("reference_feature.shape")
            # print(reference_feature.shape)
            query_reference = self.q_additional(reference_feature)
            query_reference = query_reference.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
            ip_key = self.to_k_ip(reference_text)
            ip_value = self.to_v_ip(reference_text)
            ...
            #$$ attn_mask?  
            hidden_states_reference = F.scaled_dot_product_attention(
                query_reference, ip_key, ip_value, attn_mask=None, dropout_p=0.0, is_causal=False
            )

            hidden_states_reference = hidden_states_reference.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
            hidden_states_reference = hidden_states_reference.to(query.dtype)

            ip_key = self.k_additional(hidden_states_reference)
            ip_value = self.v_additional(hidden_states_reference)
